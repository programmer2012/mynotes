...
class NotesServices {
  ...
  Future<DatabaseNote> updateNote({
    ...
    // make sure note excist
    await getNote(id: note.id);

    // update DB
    final updatesCount = await db.update(noteTable, {
      textColumn: text,
      isSyncedWithCloudCoumn: 0,
    });
  ...
  Future<DatabaseNote> createNote({required DatabaseUser owner}) async {
    ...
    // make sure owner in the datrabes wirhg the correct id
  ...
  Future<void> open() async {
    ...
    try {
      ...
      throw UnableToGetDocumentsDircetory();
    }
  }
}
...
class DatabaseNote {
  ...
  DatabaseNote.fromRow(Map<String, Object?> map)
      : id = map[idColumn] as int,
        ...
        isSyncedWithCloud =
            (map[isSyncedWithCloudCoumn] as int) == 1 ? true : false;
...
const isSyncedWithCloudCoumn = 'is_synced_with_cloud';
